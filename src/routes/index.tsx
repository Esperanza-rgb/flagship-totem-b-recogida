...
function QRScreen({ code, onReset }: { code: string; onReset: () => void }) {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center px-10 pt-[26%] pb-[8%] text-brand-white">
      ...
      <p className="mt-3 text-center text-sm leading-snug text-brand-white -mx-2">
        Para descargar tu vídeo, captura este<br />código QR con tu teléfono móvil.
      </p>
      <p className="mt-3 text-center text-sm leading-snug text-brand-white max-w-[300px] mx-auto">
      ...
      <div className="bg-brand-white rounded-md p-3 border-2 border-brand-white mt-[32px]">
        {qrDataUrl ? (
          ...
          <div className="w-[16vw] max-w-[80px] aspect-square" />
        )}
      </div>
      ...
      <button
        onClick={onReset}
        className="w-[80%] mx-auto bg-brand-white rounded-md py-4 px-6 flex items-center justify-center gap-3 shadow-lg text-brand-blue font-bold text-xl hover:brightness-95 active:scale-[0.99] transition mt-[80px]"
      >
    ...
    <div className="absolute inset-0 flex flex-col px-10 pt-[26%] pb-[10%] text-brand-white">
      ...
      <div className="mt-4 flex flex-col gap-3 mx-[16px]">
        {current.options.map((opt, i) => (
          ...
          >
            <span
              className={`shrink-0 w-5 h-5 rounded-full border-2 border-brand-white flex items-center justify-center ${
